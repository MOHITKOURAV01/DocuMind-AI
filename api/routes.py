...
async def get_user_id(request: Request, response: Response):
    user_id = request.cookies.get("dm_user_id")
    if not user_id:
        user_id = str(uuid.uuid4())
        response.set_cookie(key="dm_user_id", value=user_id, max_age=86400 * 30)
...
def auth_login(user_id: str = Depends(get_user_id)):
    ...
    res.set_cookie(key="dm_user_id", value=user_id, max_age=86400 * 30)
    return res
...
def sync_drive(body: SyncRequest = SyncRequest(), user_id: str = Depends(get_user_id)):
    ...
    for file in drive_files:
        ...
        if file_id in processed_files and processed_files[file_id] == modified_time:
            print(f"[DocuMind] Skipping unchanged file: {file_name}")
            skipped += 1
            continue

        print(f"[DocuMind] Processing: {file_name}")
        ...
        try:
            ...
            if not text.strip():
                print(f"[DocuMind] No text extracted from {file_name}, skipping.")
                continue
            ...
            print(f"[DocuMind] Error processing {file_name}: {e}")
            continue
